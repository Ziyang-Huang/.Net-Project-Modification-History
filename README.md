...
## What it does
- Scans the repo for directories that contain any of: `.bproj`, `.csproj`, `.dtproj`, `.scopeproj`, `.sqlproj`, `.vcproj`, `.vcxproj`, `.xproj`, `.sln`.
...
- `--project-type`: One or more of `.bproj`, `.csproj`, `.dtproj`, `.scopeproj`, `.sqlproj`, `.vcproj`, `.vcxproj`, `.xproj`, `.sln`. Repeat or comma‑separate. Default: all. If not all are included, the filename gains a `_type` suffix (e.g., `_csproj_vcxproj`).
